...
import numpy as np
import math
from models_densenet import DenseNet
...
class VGGActReg(nn.Module):
    '''
    VGG model. Source: https://github.com/chengyangfu/pytorch-vgg-cifar10/blob/master/vgg.py
    (in turn modified from https://github.com/pytorch/vision.git)
    '''
    def __init__(self, n_cls, half_prec, cfg):
        super(VGGActReg, self).__init__()
        self.half_prec = half_prec
        self.features = self.make_layers(cfg)
        n_out = cfg[-2]  # equal to 8*model_width
        self.classifier = nn.Sequential(
    ...
    def forward(self, x, compute_act_reg=False, act_reg_start_from=0):
        """
        act_reg_start_from: from 0 to 15
        """
        assert 0 <= act_reg_start_from <= 15

        act_reg = 0
        i = 0
        for layer in self.features:
            x = layer(x)
            if compute_act_reg and isinstance(layer, nn.ReLU):
                if i >= act_reg_start_from:
                    # act_reg += torch.mean(torch.abs(x))
                    act_reg += torch.mean(x**2)
                    # act_reg += torch.mean(x**2, axis=0).sum()
                i += 1
...
def VGG16ActReg(n_cls, model_width, half_prec):
    """VGG 16-layer model (configuration "D")"""
    w1, w2, w3, w4, w5 = model_width, 2*model_width, 4*model_width, 8*model_width, 8*model_width
    # cfg = [64, 64, 'M', 128, 128, 'M', 256, 256, 256, 'M', 512, 512, 512, 'M', 512, 512, 512, 'M']
    cfg = [w1, w1, 'M', w2, w2, 'M', w3, w3, w3, 'M', w4, w4, w4, 'M', w5, w5, w5, 'M']
    return VGGActReg(n_cls, half_prec, cfg)
...
def PreActResNet34(n_cls, model_width=64, cuda=True, half_prec=False, activation='relu', droprate=0.0, final_layer_factor=8):
    bn_flag = True
...
def get_model(model_name, n_cls, half_prec, shapes_dict, model_width, activation='relu', droprate=0.0):
    ...
    if model_name == 'vit_basic':
        ...
        model = DenseNet(nClasses=n_cls, growthRate=model_width, depth=40)
    elif model_name == 'densenet100':
        model = DenseNet(nClasses=n_cls, growthRate=model_width, depth=100)
        ...
        final_layer_factor = (shapes_dict[-1] // 32)**2 * 8  # 32 -> 8, 64 -> 32
        model = PreActResNet34(n_cls, model_width=model_width, half_prec=half_prec, activation=activation, droprate=droprate, final_layer_factor=final_layer_factor)
    ...
    elif model_name == 'vgg16_act_reg':
        model = VGG16ActReg(n_cls, model_width, half_prec)
