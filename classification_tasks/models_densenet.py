...
class Transition(nn.Module):
    ...
    def forward(self, x, return_after_relu=False):
        if return_after_relu:
...
class DenseNet(nn.Module):
    ...
    def forward(self, x, return_features=False, return_block=5):
        ...
        if return_features and return_block == 2:
            out = self.trans1(self.dense1(out), return_after_relu=True)
            return out
        else:
            out = self.trans1(self.dense1(out))
        if return_features and return_block == 3:
            out = self.trans2(self.dense2(out), return_after_relu=True)
            return out
        else:
        ...
        out = F.avg_pool2d(out, out.shape[-1]).squeeze(3).squeeze(2)
