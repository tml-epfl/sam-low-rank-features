...
def get_image_model(random_init_image):
  ...
  if random_init_image:
    params = model.init(jax.random.PRNGKey(0), jnp.ones((1, resolution, resolution, 3)), train=False)['params']
  else:
    params = vit_checkpoint.load(path)
  return model, params
